...
Theorem REV_ASSOCD_drop_prefix:
  !pfx x s d. ~MEM x (MAP SND pfx) ⇒ REV_ASSOCD x (pfx ++ s) d = REV_ASSOCD x s d
Proof
...
Proof
  gen_tac >> Induct >> rw[TYPE_SUBST_def,REV_ASSOCD_drop_prefix]
QED
...
Theorem type_size_TYPE_SUBST':
  !s l1. SUM (MAP type_size' l1) <= SUM (MAP type_size' (MAP (λa. TYPE_SUBST s a) l1))
Proof
  rw[]
  >> qspec_then `Tyapp m l1` mp_tac type_size_TYPE_SUBST
...
Proof
  ho_match_mp_tac clean_tysubst_ind
  >> rw[clean_tysubst_def,FILTER_EQ_ID]
  >> rw[EVERY_MEM]
  >> imp_res_tac (Q.ISPEC `SND` MEM_MAP_f)
  >> CCONTR_TAC
  >> fs[UNCURRY]
...
Proof
  ...
  >> rw[SUBSET_UNION,MEM_FILTER,SUBSET_DEF,MEM_MAP]
QED

Theorem clean_tysubst_FST_SND_SUBSET:
  (!s. set (MAP FST (clean_tysubst s)) ⊆ set (MAP FST s))
  ∧ !s. set (MAP SND (clean_tysubst s)) ⊆ set (MAP SND s)
Proof
  conj_tac
  >> gen_tac
  >> qspec_then `s` assume_tac clean_tysubst_SUBSET
  >> fs[SUBSET_DEF]
  >> gen_tac >> disch_then (strip_assume_tac o REWRITE_RULE[MEM_MAP])
  >> rveq
  >> first_x_assum (drule_then assume_tac)
  >> imp_res_tac (Q.ISPEC `FST:type#type->type` MEM_MAP_f)
...
Theorem is_subseq_FILTER_IMP:
  !s s' P. is_subseq s s'⇒ is_subseq s (FILTER P s')
Proof
  fs[GSYM PULL_FORALL]
  >> ho_match_mp_tac is_subseq_ind
  >> rw[]
  >> FULL_CASE_TAC
  >> fs[is_subseq_def]
  >> qmatch_goalsub_abbrev_tac `FILTER P _`
  >> first_x_assum (drule_then (qspec_then `P` mp_tac))
  >> fs[is_subseq_cons]
QED

Theorem is_subseq_FILTER =
...
Proof
  Induct >> fs[clean_tysubst_def]
  >> PairCases >> rename1`(_,h)` >> reverse (Cases_on `h`)
  >> fs[clean_tysubst_def,is_subseq_cons]
  >> FULL_CASE_TAC
  >> fs[is_subseq_cons,is_subseq_FILTER,is_subseq_cons,is_subseq_FILTER_IMP,is_subseq_def]
QED

Theorem clean_tysubst_is_subseq:
  !s1 s2. ?r1 r2. clean_tysubst (s1 ++ s2) = r1 ++ r2
    ∧ is_subseq s1 r1 ∧ is_subseq s2 r2
Proof
  strip_tac
  ...
  >- (
    ...
    >> rw[]
    >> qmatch_asmsub_rename_tac `_ = r1 ++ r2`
    >> map_every qexists_tac [`FILTER P r1`,`FILTER P r2`]
    >> fs[GSYM FILTER_APPEND,is_subseq_FILTER_IMP,is_subseq_cons]
  )
  >> strip_tac
  >> qmatch_goalsub_abbrev_tac `FILTER P _`
  >> qmatch_goalsub_abbrev_tac `l1 ++ l2`
  >> Q.REFINE_EXISTS_TAC `(_,_)::_`
  >> first_x_assum (qspecl_then [`l1`,`l2`] mp_tac)
  >> rw[is_subseq_def]
  >> qmatch_asmsub_rename_tac `_ = r1 ++ r2`
  >> map_every qexists_tac [`FILTER P r1`,`FILTER P r2`]
...
Theorem clean_tysubst_idem:
  !s. clean_tysubst (clean_tysubst s) = clean_tysubst s
Proof
  Induct >- fs[clean_tysubst_def]
  >> PairCases >> reverse (Cases_on `h1`)
  >- fs[clean_tysubst_def]
  >> rw[clean_tysubst_def] THEN_LT USE_SG_THEN ASSUME_TAC 1 2
  >- fs[GSYM clean_tysubst_FILTER]
  >> fs[FILTER_IDEM]
QED
...
Proof
  ...
  >- (
    ...
    >> dxrule_then strip_assume_tac TYPE_SUBST_MEM_MAP_SND
    >> Cases_on `MEM x (tyvars b)`
    >- (
      Cases_on `b` >- (
        qspec_then `s` assume_tac clean_tysubst_ineq
        >> gs[tyvars_def,EVERY_MEM,ELIM_UNCURRY] >> res_tac >> fs[]
      )
    ...
    >> rev_dxrule_then strip_assume_tac $ cj 1 $ REWRITE_RULE[EQ_IMP_THM] $ GSYM subtype_at_tyvars
    >> first_x_assum $ qspec_then `SND y'` assume_tac
    >> `?a. SND y' = Tyvar a` by (
      qunabbrev_tac `s'`
      >> imp_res_tac $ Q.ISPEC `SND` MEM_MAP_f
    ...
    >> drule_then (qspec_then `s'` assume_tac) subtype_at_TYPE_SUBST
    >> Cases_on `y'` >> gvs[]
    >> dxrule_at Any TYPE_SUBST_MEM'
    >> impl_keep_tac >- fs[Abbr`s'`,clean_tysubst_ALL_DISTINCT_MAP_SND] >> strip_tac
    >> drule_all_then assume_tac TYPE_SUBST_MEM' >> gvs[]
    >> qpat_x_assum `Tyvar _ = _` $ (fn x => fs[GSYM x,tyvars_def])
  )
  >> qmatch_goalsub_abbrev_tac `TYPE_SUBST s' _`
  >> rw[TYPE_SUBST_tyvars,TYPE_SUBST_compose]
  >> REWRITE_TAC $ map GSYM [TYPE_SUBST_compose,TYPE_SUBST_def]
  >> reverse $ Cases_on `MEM (Tyvar x) (MAP SND s')`
  >- fs[TYPE_SUBST_drop_all]
  >> res_tac
  >> dxrule_then strip_assume_tac TYPE_SUBST_MEM_MAP_SND >> fs[]
  >> CONV_TAC $ LAND_CONV $ REWR_CONV $ GSYM TYPE_SUBST_NIL
  >> rw[TYPE_SUBST_tyvars,REV_ASSOCD_def]
  >> irule REV_ASSOCD_NOT_MEM_drop
  ...
  unify_types_invariant orig_l l sigma =
    ...
    /\ (!s. EVERY (λ(x,y). TYPE_SUBST s x = TYPE_SUBST s y) orig_l
      ==> ?s'. EVERY (λ(x,y). (TYPE_SUBST s' o TYPE_SUBST sigma) x
        = (TYPE_SUBST s' o TYPE_SUBST sigma) y
        /\ (TYPE_SUBST s' o TYPE_SUBST sigma) x = TYPE_SUBST s x
        /\ (TYPE_SUBST s' o TYPE_SUBST sigma) y = TYPE_SUBST s y
        ) orig_l)
    /\ EVERY (UNCURRY $<>) sigma
    /\ idempotent sigma
...
Proof
  ...
  >- (
    ...
    >> qpat_x_assum `!a b. _ \/ _ ==> _` $ assume_tac o cj 1 o
      Ho_Rewrite.REWRITE_RULE[FORALL_AND_THM,DISJ_IMP_THM,IMP_CONJ_THM]
    >> fs[]
    ...
    >> qmatch_asmsub_rename_tac `TYPE_SUBST s' $ TYPE_SUBST sigma _`
    >> qexists_tac `s'` >> strip_tac >> fs[]
    >> `TYPE_SUBST s' in_x = TYPE_SUBST s' in_y` by (
      ...
      >> gs[FORALL_AND_THM,IMP_CONJ_THM]
    )
    >> `TYPE_SUBST ((ty,Tyvar a)::MAP (TYPE_SUBST [(ty,Tyvar a)] ## I) sigma)
      = TYPE_SUBST [(ty,Tyvar a)] o TYPE_SUBST sigma` by (
      rw[FUN_EQ_THM,TYPE_SUBST_compose,TYPE_SUBST_tyvars]
    ...
    >> unabbrev_all_tac
    >> (dxrule_then assume_tac TYPE_SUBST_eating
      ORELSE
        dxrule_then assume_tac $
          CONV_RULE (ONCE_DEPTH_CONV $ LAND_CONV SYM_CONV) TYPE_SUBST_eating)
    >> gs[FUN_EQ_THM]
  )
  >> conj_asm1_tac
  >- (
    rw[EVERY_MEM,MEM_MAP]
    >> fs[unify_types_invariant_def,ELIM_UNCURRY]
    >> rename[`MEM y sigma`]
    >> PairCases_on `y`
    >> drule_then assume_tac $ Q.ISPEC `SND` MEM_MAP_f
    >> qpat_x_assum `!x. _ ==> ?y. _` $ drule_then strip_assume_tac
    >> qpat_x_assum `EVERY _ _` (fn x =>
      drule_then strip_assume_tac $ SIMP_RULE std_ss [EVERY_MEM] x
      >> drule_then strip_assume_tac $ SIMP_RULE std_ss [EVERY_MEM,Once subtype_at_eq] x
    )
    >> qpat_x_assum `!x. MEM x $ MAP SND sigma ==> _` $ drule_then strip_assume_tac
    >> gvs[] >> Cases_on `y0` >> rw[TYPE_SUBST_def,REV_ASSOCD_def]
  )
  (* idempotent *)
  >- (
    fs[idempotent_thm,unify_types_invariant_def,clean_tysubst_id]
    >> `clean_tysubst sigma = sigma` by (
      irule clean_tysubst_id >> rw[]
      >> fs[GSYM EVERY_MEM]
      >> qpat_x_assum `EVERY _ $ MAP SND _` $ mp_tac o REWRITE_RULE[EVERY_MAP]
      >> match_mp_tac EVERY_MONOTONIC
      >> fs[ELIM_UNCURRY]
    )
    >> pop_assum $ fs o single
    >> dep_rewrite.DEP_REWRITE_TAC[clean_tysubst_id]
    >> conj_asm1_tac
    >- (
      fs[ALL_DISTINCT,MAP_MAP_o,o_DEF,ETA_THM,EVERY_MAP,GSYM EVERY_MEM]
      >> spose_not_then $ strip_assume_tac o REWRITE_RULE[MEM_MAP]
      >> fs[EVERY_MEM] >> res_tac >> gs[]
    )
    >> dsimp[MEM_FLAT,MEM_MAP] >> CCONTR_TAC >> fs[]
    >- (
      fs[tyvars_TYPE_SUBST,REV_ASSOCD_def]
      >> FULL_CASE_TAC >> fs[tyvars_def]
    )
    >- (
      dxrule_then assume_tac $ Q.ISPEC `SND` MEM_MAP_f
    ...
    >- (
      ...
      >- (
        imp_res_tac $ Q.ISPEC `SND` MEM_MAP_f
        >> gvs[FORALL_AND_THM,DISJ_IMP_THM]
        >> res_tac
        >> gvs[]
      )
      >> rename[`MEM x $ tyvars $ Tyvar x'`,`Tyvar x = SND yy`]
      >> PairCases_on `yy`
      >> imp_res_tac $ Q.ISPEC `SND` MEM_MAP_f
      >> imp_res_tac $ Q.ISPEC `FST` MEM_MAP_f
      >> gvs[tyvars_def]
      >> qpat_x_assum `!x. MEM (Tyvar x) $ MAP SND sigma ==> _` drule
      >> rw[PULL_EXISTS,MEM_FLAT,MEM_MAP]
...
Proof
  simp[unify_types_invariant_def,idempotent_NIL] \\ rw[]
...
Proof
  ...
  >> fs[unify_types_def,EVERY_MEM,ELIM_UNCURRY]
  >> FULL_CASE_TAC
  >> gs[MAP_EQ_f,MAP_EQ_EVERY2,LIST_REL_EVERY_ZIP,ZIP_MAP,EVERY_MAP,MAP_MAP_o,o_DEF,EVERY_MEM,ELIM_UNCURRY]
QED
...
Theorem unify_types_invariant_Tyapp:
  unify_types_invariant [(ty1,ty2)] ((Tyapp a atys,Tyapp b btys)::l) sigma
  /\ TYPE_SUBST θ (TYPE_SUBST sigma ty1) = TYPE_SUBST θ (TYPE_SUBST sigma ty2)
  ==> TYPE_SUBST θ (Tyapp a atys) = TYPE_SUBST θ (Tyapp b btys)
Proof
  rw[Excl"TYPE_SUBST_def",unify_types_invariant_def,FORALL_AND_THM,DISJ_IMP_THM,AND_IMP_INTRO]
  >> qmatch_asmsub_abbrev_tac `subtype_at (TYPE_SUBST _ ty1) p = SOME in_ty1`
  >> qmatch_asmsub_abbrev_tac `subtype_at (TYPE_SUBST _ ty2) p = SOME in_ty2`
  >> qmatch_asmsub_abbrev_tac `TYPE_SUBST θ (TYPE_SUBST sigma _)`
  >> drule_then (qspec_then `θ` assume_tac) subtype_at_TYPE_SUBST
  >> rev_drule_then (qspec_then `θ` assume_tac) subtype_at_TYPE_SUBST
  >> dxrule_then assume_tac $ cj 1 $ REWRITE_RULE[EQ_IMP_THM] subtype_at_eq
  >> gs[]
QED
...
Proof
  ...
  >- (
    rw[unify_types_def]
    >> qmatch_assum_abbrev_tac `unify_types_invariant _ ((Tyapp a atys,Tyapp b btys)::_) _`
    >> `a = b /\ LENGTH atys = LENGTH btys` by (
      gs[unifiable_def]
      >> drule_all $ unify_types_invariant_Tyapp
      >> fs[TYPE_SUBST_def,LIST_EQ_REWRITE]
    )
    >> fs[]
    >- (
      first_x_assum irule
      >> drule_then (irule_at Any) unify_types_invariant_pres1 >> fs[]
    )
    >- (
      first_x_assum irule
      >> drule_then (irule_at Any) unify_types_invariant_pres2 >> fs[]
    )
  )
  >> strip_tac
  >- (
    rpt strip_tac
    >> dxrule_then assume_tac unify_types_invariant_pres3
    >> first_x_assum dxrule_all >> fs[unify_types_def]
  )
  >> rw[unify_types_def]
  >- (
    first_x_assum irule
  ...
  >- (
    drule_at Any unify_types_complete_cyclic_non_unifiable
    >> CCONTR_TAC
    >> fs[unifiable_def,unify_types_invariant_def,tyvars_def,FORALL_AND_THM,DISJ_IMP_THM]
    >> NTAC 2 (
...
Proof
  rw[EQ_IMP_THM,IS_SOME_EXISTS]
  >- (
    qmatch_goalsub_abbrev_tac `unify_types l _`
    >> qspec_then `l` assume_tac unify_types_invariant_init
    >> qunabbrev_tac `l`
    >> dxrule_then assume_tac unify_types_complete_step
    >> gs[TYPE_SUBST_NIL]
  )
  >> fs[unifiable_def]
  >> drule_then (irule_at Any) unify_types_sound
QED
...
Proof
  ...
  >> qspec_then `l` assume_tac unify_types_invariant_init
  >> qunabbrev_tac `l`
  >> dxrule unify_types_complete_step
  >> impl_tac
  >- (fs[TYPE_SUBST_NIL,unifiable_def] >> drule_then (irule_at Any) unify_types_sound)
  >> fs[]
QED
...
Proof
  rw[terminating_def] >> strip_tac >>
  imp_res_tac WF_NOT_REFL >>
  fs[inv_DEF]
QED
...
Definition weak_terminating_def:
  weak_terminating R = ¬?x. !n. ?y. (NRC R (SUC n) x y)
End

Theorem weak_terminating_IMP_terminating:
   weak_terminating R ==> terminating R
Proof
  rw[weak_terminating_def,terminating_def,prim_recTheory.WF_IFF_WELLFOUNDED,prim_recTheory.wellfounded_def,inv_DEF] >>
  CCONTR_TAC >> fs[] >>
  `!n. NRC R n (f 0) (f n)`
...
Proof
  rpt strip_tac
  >> match_mp_tac weak_terminating_IMP_terminating
  >> rw[weak_terminating_def]
  >> CCONTR_TAC >> fs[]
...
Proof
  ...
  drule_then(qspec_then `normalise_tyvars ## normalise_tvars` mp_tac) WF_inv_image >>
  simp[inv_image_def,PULL_EXISTS] >>
  strip_tac >>
  drule_then match_mp_tac WF_SUBSET >>
  rw[inv_DEF] >> metis_tac[]
QED
...
Theorem terminating_measure_ext:
  terminating R /\
  (!x y. R x y ==> R' x y) /\
  (!x y. R' x y ==> (R x y /\ m y <= (m x):num) \/ m y < (m x):num) ==>
  terminating R'
Proof
  rw[terminating_def,prim_recTheory.WF_IFF_WELLFOUNDED,prim_recTheory.wellfounded_def,inv_DEF] >>
  `?n. n = m(f 0)` by simp[] >>
  pop_assum mp_tac >>
  qid_spec_tac `f` >> qid_spec_tac `n` >>
  ho_match_mp_tac COMPLETE_INDUCTION >>
  rw[] >>
  ...
  `m (f n) <= m (f 0)`
    by(last_x_assum mp_tac >>
       PRED_ASSUM is_forall mp_tac >>
        `?i. i = 0:num` by simp[] >>
       `i <= n` by simp[] >>
       PRED_ASSUM is_eq (fn thm => simp[GSYM thm]) >>
       last_x_assum mp_tac >>
       pop_assum mp_tac >>
       rpt(pop_assum kall_tac) >>
       Induct_on `n - i` >- (rw[] >> imp_res_tac LESS_EQUAL_ANTISYM >> fs[]) >>
       rw[] >>
       Cases_on `n` >> fs[] >> rename1 `SUC v = SUC n - i` >>
       `v = n - i` by intLib.COOPER_TAC >>
       qpat_x_assum `!n i'. _ = _ - (_:num) ==> _` drule >>
       simp[] >>
       first_x_assum(qspec_then `n` mp_tac) >>
       simp[] >>
       strip_tac >> res_tac >> res_tac >> fs[]) >>
    last_x_assum(qspec_then `m (f(SUC n))` mp_tac) >>
    impl_tac >- simp[] >>
    disch_then(qspec_then `f o $+ (SUC n)` mp_tac) >>
    impl_tac >- simp[] >>
    simp[] >>
    strip_tac >>
    metis_tac[ADD_SUC,ADD_SYM]
QED
...
Termination
  ...
  rw[type_size_def] >>
  fs[MEM_SPLIT] >>
  fs[type1_size_append,type_size_def]
End
...
Proof
  ...
  rfs[] >>
  fs[LIST_TO_SET_MAP] >>
  simp[set_foldr_list_union] >>
  match_mp_tac LESS_EQUAL_ANTISYM >>
  conj_tac >-
    (match_mp_tac SUBSET_MAX_SET >>
     rw[SUBSET_DEF,PULL_EXISTS,MEM_FLAT,MEM_MAP] >>
     metis_tac[]) >>
  pred_setLib.MAX_SET_elim_tac >>
  fs[] >>
  ...
  rw[] >>
  fs[] >>
  qpat_x_assum `!a. _ ==> MAX_SET _ <= MAX_SET _` (qspec_then `y` mp_tac) >>
  rw[] >>
...
Proof
  ...
  qexists_tac `tysof l2` >>
  qexists_tac `tmsof l2` >>
  rw[] >>
...
Theorem extends_update_ok_TypeDefn':
  ...
  MEM (TypeDefn name pred abs rep) ctxt ==>
  type_ok (tysof ctxt) (domain(typeof pred))
Proof
  ...
  fs[term_ok_def] >>
  fs[] >>
  qpat_x_assum `type_ok _ (Fun (typeof witness) rty)` mp_tac >>
  rw[type_ok_def] >>
  match_mp_tac type_ok_extend >>
  HINT_EXISTS_TAC >>
  rw[] >>
...
Theorem extends_update_ok_ConstSpec:
  ...
  MEM (name,cdefn) cl
  ==>
  term_ok (sigof ctxt) cdefn /\ type_ok (tysof ctxt) (typeof cdefn)
Proof
  rw[MEM_SPLIT] >>
  FULL_SIMP_TAC bool_ss [GSYM APPEND_ASSOC] >>
  imp_res_tac extends_NIL_DISJOINT >>
  ...
  TRY(
    ...
    match_mp_tac(CONJUNCT2 SUBMAP_FUNION_ID) >>
    simp[] >>
    fs[constspec_ok_def,ALL_DISTINCT_APPEND,DISJOINT_SYM] >>
    imp_res_tac extends_NIL_DISJOINT >>
    fs[] >>
    rw[] >> fs[] >> NO_TAC) >>
  TRY(match_mp_tac type_ok_extend >>
      HINT_EXISTS_TAC >>
      rw[] >>
      match_mp_tac(CONJUNCT2 SUBMAP_FUNION_ID) >>
      simp[])
...
Theorem extends_update_ok_NewConst:
  ctxt extends [] /\
  MEM (NewConst c ty) ctxt
  ==>
  term_ok (sigof ctxt) (Const c ty) /\ type_ok (tysof ctxt) ty
Proof
  rw[MEM_SPLIT] >>
  FULL_SIMP_TAC bool_ss [GSYM APPEND_ASSOC] >>
  imp_res_tac extends_NIL_DISJOINT >>
  ...
  drule extends_NIL_CONS_updates >>
  rw[updates_cases] >>
  rw[term_ok_def,FLOOKUP_UPDATE,FLOOKUP_FUNION] >>
...
Theorem extends_update_ok_ConstSpec':
  ...
  MEM (name,cdefn) cl
  ==>
  (!x. MEM x (tvars cdefn) ==> MEM x (tyvars (typeof cdefn)))
Proof
  strip_tac >>
  rpt(qpat_x_assum `MEM _ _` (strip_assume_tac o REWRITE_RULE [MEM_SPLIT])) >> fs[] >>
  rveq >> fs[] >>
  FULL_SIMP_TAC bool_ss [GSYM APPEND_ASSOC] >>
  imp_res_tac extends_NIL_DISJOINT >>
  drule extends_APPEND_NIL >>
...
Theorem allCInsts_term_ok:
  MEM c (allCInsts trm) /\
  term_ok sig trm ==>
  term_ok sig c
Proof
  Induct_on `trm` >>
  rw[allTypes_def,tvars_def,allCInsts_def,
     List.nth(CONJUNCTS term_ok_def,2),
     List.nth(CONJUNCTS term_ok_def,3)] >>
  res_tac >> fs[] >>
  Cases_on `t` >> fs[allCInsts_def] >>
  TRY(FULL_CASE_TAC >> fs[])
QED

Theorem allCInsts_is_Const:
  MEM c (allCInsts trm) ==>
  ?name ty. c = Const name ty
Proof
  Induct_on `trm` >>
  rw[allCInsts_def] >> res_tac >> fs[] >>
  Cases_on `t` >> fs[allCInsts_def] >>
  TRY(FULL_CASE_TAC >> fs[])
QED

Theorem allCInsts_tyvars:
  MEM (Const c ty) (allCInsts trm) /\ MEM v (tyvars ty) ==>
  MEM v (tvars trm)
Proof
  Induct_on `trm` >>
...
Proof
  ...
  >-
    (disj1_tac >>
     ...
     rveq >>
     `type_ok (tysof ctxt) ty` by(fs[term_ok_def]) >>
